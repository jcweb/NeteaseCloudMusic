...
//        String encText = aesEncrypt(aesEncrypt(text, nonce), secKey);
    ...
    private static Map<String,String> aesEncrypt(String text,String mode,String key ){
        Map map = new HashMap();
        try {
            IvParameterSpec iv = new IvParameterSpec("0102030405060708".getBytes("UTF-8"));
            SecretKeySpec skeySpec = new SecretKeySpec(key.getBytes("UTF-8"), "AES-128-"+mode);

            Cipher cipher = Cipher.getInstance("AES/CBC/PKCS5Padding");
            cipher.init(Cipher.ENCRYPT_MODE, skeySpec, iv);

            byte[] encryptedText = cipher.doFinal(text.getBytes());
                   byte[] encryptedKey = cipher.update(text.getBytes());

            map.put(PARAMS, Base64Encoder.encode(encryptedKey));
            map.put(ENCSECKEY, Base64Encoder.encode(encryptedText));
            return map;
        } catch (Exception e) {
            return map;
        }
    ...
    * 部分接口的加密方式有区别，入口一不一样
    * */
    private static Map<String,String> cryptoAnother(String text){
        Map<String, String> map = new HashMap<String, String>();
         map = aesEncrypt(text,"ecb", linuxapiKey);
        return map;
    }
